...
def plot_cross_species_category_heatmap(mapping_both, species_1, species_2):
    df = mapping_both.loc[mapping_both[f"{species_2}_homolog_orthology_type"] == 'ortholog_one2one'].groupby(
        [f"spec_category_{species_1}"])[f"spec_category_{species_2}"].value_counts().to_frame().add_suffix('_count').reset_index()
    ...
    df['Species1_Percentage'] = df.apply(
        lambda row: (row[f"spec_category_{species_2}_count"] / species1_counts[row[f"spec_category_{species_1}"]]) * 100, axis=1)
    df['Species2_Percentage'] = df.apply(
        lambda row: (row[f"spec_category_{species_2}_count"] / species2_counts[row[f"spec_category_{species_2}"]]) * 100, axis=1)
    ...
    order = ['lowly expressed', 'low cell type specificity', 'group enhanced', 'cell type enhanced', 'group enriched',
             'cell type enriched']
    fig = px.imshow(heatmap_data, text_auto=True, width=700, height=600)
